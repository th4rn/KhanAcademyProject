...
-- Show all of the data from the NFL picks table

select * from khanacademy.picks;

-- Find all of the unique teams who participated in draft

select distinct 
picked_by
from khanacademy.picks;

-- Find the number of teams who participated in the draft

select count(distinct picked_by)
as teams
from khanacademy.picks;
...
-- Find the number of unique positions of players in the draft

select count(distinct position)
as positions
from khanacademy.picks;

-- Find the number of players selected in draft grouped by position
...
-- Find the number of different positions of players which each team signed

select picked_by, count(distinct position) as "Number of different positions signed"
from khanacademy.picks
group by picked_by
order by count(distinct position) desc;

-- Find which positions the New Orleans team signed in the draft

select picked_by, position 
from khanacademy.picks
where picked_by = "New Orleans";

-- Find out which teams signed players who identifed as the position "WR"
